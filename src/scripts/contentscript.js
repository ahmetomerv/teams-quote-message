...
function addQuoteMessage() {
   ...
   if (threadBodyMediaList) {
      threadBodyMediaList.forEach(threadBody => {
         const threadBodyTopRow = threadBody.querySelector('.top-row-text-container');
         ...
         if (threadBody && !threadBodyTopRow.hasAttribute('data-hasquote')) {
            if (threadBodyTopRow) {
               ...
               quoteMessageButton.addEventListener('click', quoteMessageButtonClickHandler);
            }
         }
      });
   }
...
function sendButtonClickHandler() {
   const replyMessageCloseButton = document.querySelector('.teams-custom-quote-message-close');
